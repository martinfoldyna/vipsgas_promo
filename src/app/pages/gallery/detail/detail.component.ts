...
export class DetailComponent implements OnInit {
  ...
  load() {
    ...
    this.galleryService
      .loadSectionById(this.id)
      .then((section) => {
        this.section = section;
        if (this.section.images) {
          this.section.images = this.section.images.sort((a, b) =>
            a.name.toLowerCase() > b.name.toLowerCase() ? 1 : -1
          );
        }
        console.log(this.section);
        this.imagesService
          .getImage('gallery', this.section.thumbnail.name)
          .then((image) => {
  ...
  upload(form) {
    ...
    this.uploadImages()
      ...
      .catch((err) => {
        this.toastr.danger(
          err ? JSON.stringify(err) : 'Během nahrávání obrázků došlo k chybě.',
          'Chyba'
        );
      });
  }
    ...
    return new Promise<boolean>((resolve, reject) => {
      ...
      if (this.newImages.length > 0) {
        for (let i = 0; i < this.newImages.length; i++) {
          ...
          this.galleryService
            .uploadImagesToSection(this.section, image)
            .then((uploadImages) => {
              console.log(uploadImages);
              uploadedImages += 1;
              if (this.newImages.length - 1 === uploadedImages) resolve(true);
            })
            .catch((err) => {
              reject(err);
            });
  ...
  updateSection() {
    this.galleryService
      .updateSection('gallery', this.section)
      .then((response) => {
      ...
      .catch((err) => {
        this.toastr.danger(
          err ? JSON.stringify(err) : 'Během upravování sekce došlo k chybě.',
          'Chyba'
        );
      });
  }
