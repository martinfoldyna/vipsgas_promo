...
export class EditCategoryComponent implements OnInit {
  ...
  setupFileReader(file) {
    return new Promise<{ name: string; blob: Blob }>((resolve, reject) => {
      ...
      reader.onload = (e: any) => {
        ...
        this.imagesService
          .compressFile(e.target.result, fileName, 50)
  ...
  updateCategory() {
    ...
    this.category.categoryTypes = _.union(
      this.category.categoryTypes,
      this.typeOptions
    );

    this.category.categoryPositions = _.union(
      this.category.categoryPositions,
    ...
    this.categoryService
      ...
      .catch((err) => {
        ...
        this.toastr.danger(
          err
            ? JSON.stringify(err)
            : 'Během komunikace s datábází došlo k chybě',
          'Chyba'
        );
      });
  }
