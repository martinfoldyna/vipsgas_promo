...
import {Product, ProductPosition, ProductType} from "../../../@core/data/product";
...
export class ProductCategoryOverviewComponent implements OnInit {
    ...
    reader.onload = (e: any) => {
      this.imagesService.compressFile(e.target.result, file.name, 55).then(compressedImage => {
        if(compressedImage) {
          ...
          this.thumbnail = {name: this.generateRandomString(), blob: compressedImage.blob};
        }
      }).catch(err => {
        console.log(err);
      })
  ...
  uploadCategory() {
    ...
    this.categoryService.createCategory(categoryData).then(response => {
      this.toastr.success('', 'Nová kategorie byla úspěšně vytvořena.');
    }).catch(err => {
      this.toastr.danger(err ? JSON.stringify(err) : 'Během komunikace s databází došlo k cyhbě.', 'Chyba');
    })
  }
        ...
        this.productsService.getProductsByCategory(categoryID).then(allProducts => {
          ...
          if (allProducts && allProducts.length > 0) {
            for (let product of allProducts) {
              this.generalService.deleteItem('products', product.id).then(item => {
                i += 1;
              }).catch(err => {
                this.toastr.danger(err.message ? err.message : 'Během odstaňování produktů došlo k chybě.', 'Chyba');
              });
            }
          ...
          if (canDelete) {
            this.categoryService.removeCategory(thisCatgegory).then(response => {
              this.toastr.success('Kategorie byla úspěšně smazána.');
            }).catch(err => {
              this.toastr.danger(err.message ? err.message : 'Během odstaňování kategorie došlo k chybě.', 'Chyba');
            });
          }
        }).catch(err => {
          this.toastr.danger(err.message ? err.message : 'Během načítání produtků kategorie došlo k chybě.', 'Chyba');
        });
      }
    });
